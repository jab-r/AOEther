...
- DSD-capable ALSA capture path on the talker. M6 added native DSD on the wire, but the ALSA capture source (`talker/src/audio_source_alsa.c`) was still hard-locked to `S24_3LE`, blocking the snd-aloop bridge pattern from carrying native DSD even though every other piece was in place. The follow-up adds `--capture-format {pcm_s24_3le,dsd_u8}` so a DSD-capable UPnP renderer (upmpdcli + MPD) can write native DSD into the loopback and the talker can read it as DSD. See [`docs/recipe-upnp.md`](recipe-upnp.md) — that recipe's audiophile path depends on this flag. Wider DSD ALSA variants (`dsd_u16_*`, `dsd_u32_*`) on the capture side are deferred until a real-world need surfaces; for the loopback bridge the operator controls both sides and can pin the renderer to `dsd_u8`.
...
- DSD is not carried in Mode 4 — AES67 is PCM-only on the wire. Multichannel DSD streams use Modes 1 / 3 instead, which carry arbitrary channel counts via M6 + M8 packet splitting (see §Bandwidth budget). Single-multichannel-DAC DSD (Exasound e62 / e68, Merging NADAC, and similar) is the supported endpoint configuration today. Distributed multichannel DSD across multiple free-running USB DSD DACs has the same drift problem as distributed multichannel PCM and the same answer in M11 Path C hardware (a PHC-slaved fractional-N PLL clocks I2S PCM or native-DSD output identically — only the divider ratio differs); it is not architecturally harder than the PCM case.
...
**Goal:** A documented reference design for a DIY Milan or AES67 endpoint whose audio output clock is physically slaved to the PTP hardware clock, targeting builders who want the Path C architecture (from M10) without buying Merging- or L-Acoustics-class hardware. Framed as a community-driven project under AOEther's umbrella rather than a required milestone: AOEther supplies the software path (M5 AVTP receiver for Milan; M9 + M10 RTP receiver for AES67 — both shipped), M11 supplies the hardware reference and driver-integration notes.

**Why Path C and not plain ClearFog + USB DAC:** A Marvell ClearFog CN913x with `ptp4l` + `phc2sys` running AOEther's M5 AVTP or M9 RTP listener over a USB DAC is trivially buildable and supported on main today — but the USB DAC's internal crystal is free-running, so network-side PTP alignment does not translate to audio-clock alignment at the DAC. Across N such endpoints the clock-domain story reverts to the free-running-crystal problem that distributed multichannel otherwise avoids. Path C is the engineering project of making the endpoint's audio output clock actually derive from the PHC — which is what Merging, Neumann, Genelec, Dutch & Dutch, and L-Acoustics Creations have all engineered into their own boxes, and what keeps their arrays sample-coherent. gPTP (Milan, 802.1AS) and PTPv2 default profile (AES67) both discipline the same Linux PHC; downstream of the PHC the audio-clock chain is invisible to which protocol set the time. The hardware reference is therefore identical for either transport — only the `ptp4l` config and the userspace listener (M5 vs M9) differ.

**Scope (expand as contributors deliver):** Reference schematic and BOM for a ClearFog CN913x carrier with I2S output to a well-regarded DAC chip (ES9038Q2M or AK4493-class), MCLK derived from the PHC via a fractional-N PLL driven from a GPIO or dedicated PHC-out pin. Kernel and device-tree notes for exposing the PHC-derived MCLK to the ALSA SoC driver so that sample-rate changes reconfigure the PLL coherently. The PLL/MCLK chain works equally for a Milan listener (gPTP-disciplined PHC) or an AES67 listener (PTPv2-disciplined PHC); the I2S DAC chain is the same. Optional variant: AES3 output from a transmitter IC whose bit clock is the PHC-derived MCLK, feeding a powered speaker that AES3-slaves its internal DAC. Validation: long-soak drift comparison between a Path-C endpoint and a reference commercial endpoint (Genelec SAM on the AES67 side, an L-Acoustics Creations cabinet on the Milan side) across a multi-day session, quantifying that the DIY path achieves parity within the PTP sync budget.

**Out of scope for M11 (explicit):** Productization, PCB fabrication service, or warranty. Non-ClearFog platforms — contributors can port, but the reference stays singular. USB-DAC-based endpoints — a plain USB DAC reintroduces a free-running crystal and does not satisfy Path C by definition, regardless of transport.

**Status:** [open, community-driven] indefinitely. AOEther core maintenance does not depend on M11 closing; M5 / M9 / M10 are sufficient for Paths A and B on either transport, which cover the realistic commercial-hardware deployment.
