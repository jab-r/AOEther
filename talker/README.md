...
- `--source testtone|wav|alsa|dsdsilence|dsf|dff` — default `testtone` for PCM, `dsdsilence` for DSD. `testtone` is 1 kHz sine at −6 dBFS. `dsdsilence` emits the DSD idle pattern (`0x69`) — silent on a real DAC, used to verify the wire path. `dsf` / `dff` read Sony DSF or Philips/Sony DSDIFF files for real DSD playback.
- `--file PATH` — WAV file when `--source wav`, `.dsf` with `--source dsf`, `.dff` with `--source dff`. Files loop. DSF/DFF files are validated against `--format` / `--channels`; DST-compressed DFF files are rejected.
